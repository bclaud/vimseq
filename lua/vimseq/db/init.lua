...
function M.upsert_note(path, title, content_hash)
  ...
  local existing = db:eval("SELECT id FROM notes WHERE path = ?", { path })
...
function M.get_note_by_path(path)
  local db = M.get()
  local result = db:eval("SELECT * FROM notes WHERE path = ?", { path })
...
function M.get_note_by_title(title)
  local db = M.get()
  local result = db:eval("SELECT * FROM notes WHERE LOWER(title) = LOWER(?)", { title })
  if result and #result > 0 then
    return result[1]
  end
  return nil
...
function M.get_all_notes()
  local db = M.get()
  return db:eval("SELECT * FROM notes ORDER BY modified_at DESC") or {}
end
...
function M.get_content_hash(path)
  local db = M.get()
  local result = db:eval("SELECT content_hash FROM notes WHERE path = ?", { path })
  if result and #result > 0 then
